...
namespace AWMS.app.Forms.RibbonSearch
{
    public partial class frmSearchItem : XtraForm
    {
        ...
        public frmSearchItem(ISearchDapperRepository searchDapperRepository)//IRequestDapperRepository requestDapperRepository,)
        {
            InitializeComponent();
        ...
        private void LoadDataIntoGrid3()
        {
            try
            {
                gridControl1.DataSource = _searchDapperRepository.SearchItemFillGridWithDataReader();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error loading data: " + ex.Message);
